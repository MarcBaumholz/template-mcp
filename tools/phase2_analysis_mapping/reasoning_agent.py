...
from tools.phase2_analysis_mapping.get_direct_api_mapping_prompt_optimized import get_api_spec_with_direct_llm_query_optimized as get_api_spec_with_direct_llm_query, CONTEXT_WINDOW_CHAR_LIMIT
from tools.phase1_data_extraction.upload_api_specification import (
...
def _extract_field_mappings_from_text(text: str) -> Dict[str, str]:
    """Extract field mappings from markdown-like tables in text.

    Looks for a header row containing Source/Target columns and parses subsequent rows.
    Returns a dict mapping source_field -> target_field (or '?' if unknown).
    """
    if not text:
        return {}
    ...
    for line in lines:
        if "|" not in line:
            # reset on non-table lines to avoid bleeding across sections
            in_table = False
            header_indices = {}
            continue

        cells = [c.strip() for c in line.split("|")]
        # Drop empty leading/trailing cells if user used markdown borders
        ...
        if all(set(c.strip(" -:")) == set() for c in cells):
            # Only meaningful if we already saw a header
            in_table = bool(header_indices)
            continue

        # Header detection
        if not header_indices:
            normalized = [normalize_header(c) for c in cells]
            source_candidates = {"source", "source field", "field", "sourcefield"}
            target_candidates = {"target", "target field", "api field", "mapped field", "targetfield"}

            src_idx = next((i for i, c in enumerate(normalized) if c in source_candidates), None)
            tgt_idx = next((i for i, c in enumerate(normalized) if c in target_candidates), None)

            if src_idx is not None and tgt_idx is not None:
                header_indices = {"source": src_idx, "target": tgt_idx}
                # not yet in table until we pass a separator or next data row
                continue

        # Data rows
        if header_indices:
            try:
                source_cell = cells[header_indices["source"]].strip().strip("`*_")
                target_cell = cells[header_indices["target"]].strip().strip("`*_")
                if source_cell:
                    mappings[source_cell] = target_cell if target_cell else "?"
            except Exception:
                # Ignore malformed row and continue robustly
                continue
...
async def reasoning_agent(
    ...
    try:
        ...
        # Step: Field-mapping verification using verify_api_specification
        field_verification_md = "ℹ️ No field mappings detected in analysis; skipping field verification."
        try:
            # Parse mappings from both the source analysis and the mapping response
            mappings_from_analysis = _extract_field_mappings_from_text(analysis_content)
            mappings_from_mapping = _extract_field_mappings_from_text(mapping_response)
            merged_mappings: Dict[str, str] = {}
            # prefer mapping_response entries over analysis ones
            merged_mappings.update(mappings_from_analysis)
            merged_mappings.update(mappings_from_mapping)

            # Filter out empty keys and normalize unknown targets
            merged_mappings = {k: (v if v else "?") for k, v in merged_mappings.items() if k}

            if merged_mappings:
                field_verification_md = verify_spec(
                    api_spec_path=api_spec_path,
                    field_mappings=json.dumps(merged_mappings),
                    verification_type="comprehensive",
                    output_format="markdown",
                )
        except Exception as _e_ver:
            field_verification_md = f"❌ Field mapping verification failed: {_e_ver}"
...
## 🔐 Field Mapping Verification (spec-level)

{field_verification_md}
...
*Generated by Simple Reasoning Agent with integrated Proof Tool*
        ...
        # Phase 2: Enhanced Hallucination Checker
        print("🔍 Running enhanced hallucination checker...")
        try:
            hallucination_result = enhanced_hallucination_checker(
                api_spec_path=api_spec_path,
            ...
            # Check if hallucination checker failed
            if "error" in hallucination_result:
                report_content += f"""

---
## 🔍 Enhanced Hallucination Checker
❌ **Error**: {hallucination_result['error']}

**Action**: Manual verification required. Check the mapping analysis manually against the API specification.
"""
            else:
                # Add hallucination checker results to main report
                verification_status = "✅ ALL ENDPOINTS VERIFIED" if hallucination_result["all_verified"] else "⚠️ SOME ENDPOINTS NEED INVESTIGATION"
...
- **Total Endpoints Claimed**: {hallucination_result['total_claimed']}
- **✅ Verified**: {hallucination_result['verified_count']}
- **❌ Potentially Hallucinated**: {hallucination_result['unverified_count']}
- **Verification Rate**: {hallucination_result['verification_percentage']}%

### 📄 Detailed Report
**File**: `{hallucination_result['verification_file_path']}`

### 🎯 Key Findings
"""
                
                if hallucination_result["all_verified"]:
                    report_content += "🎉 **Excellent**: All claimed endpoints are verified against the OpenAPI specification. No hallucinations detected.\n\n"
                    report_content += "**Confidence Level**: HIGH - You can proceed with implementation using these verified endpoints.\n\n"
                elif hallucination_result["verified_count"] > hallucination_result["unverified_count"]:
                    report_content += "✅ **Good**: Most endpoints are verified. Focus on investigating the few potentially hallucinated ones.\n\n"
                    report_content += f"**Action Required**: Investigate {hallucination_result['unverified_count']} unverified endpoints using the detailed report.\n\n"
                elif hallucination_result["verified_count"] == 0:
                    report_content += "⚠️ **Warning**: No endpoints were verified. This suggests significant hallucinations or spec issues.\n\n"
                    report_content += "**Action Required**: Manual investigation needed. Check the detailed report for RAG query suggestions.\n\n"
                else:
                    report_content += "⚠️ **Mixed Results**: Some endpoints verified, others need investigation.\n\n"
                    report_content += f"**Action Required**: Investigate {hallucination_result['unverified_count']} unverified endpoints.\n\n"
                
                report_content += "### 🔍 Next Steps for Unverified Endpoints\n"
                report_content += "1. **Review the detailed report** for specific endpoint issues\n"
                report_content += "2. **Run suggested RAG queries** to investigate unverified endpoints\n"
                report_content += "3. **Compare with API documentation** for endpoint patterns\n"
                report_content += "4. **Update mappings** based on verified endpoints only\n"
                report_content += "5. **Document assumptions** for any unverified endpoints you decide to use\n\n"
                
                # Auto re-run direct mapping verification focused on unverified endpoints (if any)
                if hallucination_result["unverified_count"] > 0:
                    try:
                        ...
                        # Get unverified endpoints from the hallucination checker
                        all_text = (mapping_response or "") + "\n" + (verification_response or "")
                        claimed_endpoints = _extract_claimed_endpoints(all_text)
                        spec_paths = _load_spec_paths(api_spec_path)
                        verification_result = _verify_claimed_endpoints(claimed_endpoints, spec_paths)
                        
                        for e in verification_result["unverified"]:
...
def enhanced_hallucination_checker(
    api_spec_path: str,
    mapping_text: str,
    verification_text: str,
    output_directory: str,
    ...
    """
    Enhanced hallucination checker that provides detailed endpoint verification.
    
    This function:
    1. Extracts all claimed endpoints from mapping and verification text
    2. Verifies each endpoint against the OpenAPI specification
    3. Provides detailed feedback on what's verified vs hallucinated
    4. Suggests RAG queries for investigating hallucinated endpoints
    5. Returns comprehensive verification results
    
    Args:
        api_spec_path: Path to the OpenAPI specification
        mapping_text: Main mapping analysis text
        verification_text: Additional verification text
        output_directory: Directory to save verification reports
        collection_name: RAG collection name for suggested queries
    
    Returns:
        Dict with verification results, file paths, and suggested actions
    """
    try:
        # Load OpenAPI spec paths
        spec_paths = _load_spec_paths(api_spec_path)
        
        # Extract all claimed endpoints from both texts
        all_text = (mapping_text or "") + "\n" + (verification_text or "")
        claimed_endpoints = _extract_claimed_endpoints(all_text)
        ...
        verification_report = f"""# 🔍 Enhanced Hallucination Checker Report

## 📊 Verification Summary
- **Timestamp**: {timestamp}
- **API Specification**: `{api_spec_path}`
- **Total Endpoints Claimed**: {len(claimed_endpoints)}
- **✅ Verified Endpoints**: {len(verified)}
- **❌ Potentially Hallucinated**: {len(unverified)}

---

## ✅ Verified Endpoints (Found in OpenAPI Spec)

"""
        
        if verified:
            for i, endpoint in enumerate(verified, 1):
                verification_report += f"{i}. `{endpoint['method']} {endpoint['path']}`\n"
        else:
            verification_report += "No verified endpoints found.\n"
        
        verification_report += "\n---\n\n## ❌ Potentially Hallucinated Endpoints\n\n"
        
        if unverified:
            verification_report += "**These endpoints were NOT found in the OpenAPI specification:**\n\n"
            for i, endpoint in enumerate(unverified, 1):
                verification_report += f"{i}. `{endpoint['method']} {endpoint['path']}`\n"
                verification_report += f"   - **Reason**: {endpoint.get('reason', 'not_in_spec_or_method_mismatch')}\n"
                verification_report += f"   - **Action**: Investigate with RAG queries below\n\n"
        else:
            verification_report += "🎉 **All claimed endpoints are verified!** No hallucinations detected.\n"
        
        # Generate RAG investigation suggestions for unverified endpoints
        if unverified:
            verification_report += "\n---\n\n## 🔍 RAG Investigation Suggestions\n\n"
            verification_report += "**Use these MCP tool calls to investigate potentially hallucinated endpoints:**\n\n"
            ...
            for i, endpoint in enumerate(unverified, 1):
                ...
                queries = [
                    f'"{method} {path}"',
                    f'"{path}" endpoint',
                    f'"{method}" method "{path.split("/")[-1]}"',
                    f'create "{path.split("/")[-1]}"',
                    f'submit "{path.split("/")[-1]}"'
                ]
                
                verification_report += f"### Investigation {i}: `{method} {path}`\n\n"
                verification_report += "**Suggested RAG Queries:**\n\n"
                
                for j, query in enumerate(queries, 1):
                    verification_report += f"{j}. ```json\n"
                    verification_report += f"{{\n"
                    verification_report += f'  "tool": "query_api_specification",\n'
                    verification_report += f'  "arguments": {{\n'
                    verification_report += f'    "query": {query},\n'
                    verification_report += f'    "collection_name": "{collection_name}",\n'
                    verification_report += f'    "limit": 10,\n'
                    verification_report += f'    "score_threshold": 0.3,\n'
                    verification_report += f'    "current_path": "{output_directory}"\n'
                    verification_report += f'  }}\n'
                    verification_report += f"}}\n```\n\n"
                
                verification_report += "**Manual Investigation Steps:**\n"
                verification_report += "1. Check if the endpoint exists with a different HTTP method\n"
                verification_report += "2. Look for similar endpoints with different path patterns\n"
                verification_report += "3. Verify the endpoint name spelling and case sensitivity\n"
                verification_report += "4. Check if it's a deprecated or versioned endpoint\n\n"
        
        # Add overall recommendations
        verification_report += "\n---\n\n## 🎯 Recommendations\n\n"
        
        if len(verified) > len(unverified):
            verification_report += "✅ **Good News**: Most endpoints are verified. Focus on investigating the few unverified ones.\n\n"
        elif len(verified) == 0:
            verification_report += "⚠️ **Warning**: No endpoints were verified. This suggests either:\n"
            verification_report += "- The API specification is incomplete or outdated\n"
            verification_report += "- The mapping analysis contains significant hallucinations\n"
            verification_report += "- The endpoint patterns don't match the spec format\n\n"
        else:
            verification_report += "⚠️ **Mixed Results**: Some endpoints verified, others need investigation.\n\n"
        
        verification_report += "**Next Steps:**\n"
        verification_report += "1. Run the suggested RAG queries above for unverified endpoints\n"
        verification_report += "2. Compare results with the original mapping analysis\n"
        verification_report += "3. Update mappings based on verified endpoints only\n"
        verification_report += "4. Document any assumptions about unverified endpoints\n"
        verification_report += "5. Consider reaching out to API documentation for clarification\n\n"
        
        verification_report += "---\n\n*Generated by Enhanced Hallucination Checker*"
        
        # Save verification report
        verification_file_path = save_report(output_directory, verification_report, "enhanced_hallucination_check")
        
        # Generate summary for main report
        summary = {
            "total_claimed": len(claimed_endpoints),
            "verified_count": len(verified),
            "unverified_count": len(unverified),
            "verification_file_path": verification_file_path,
            "all_verified": len(unverified) == 0,
            "verification_percentage": round((len(verified) / len(claimed_endpoints) * 100) if claimed_endpoints else 0, 1)
        }
        ...
        return {
            "error": f"Hallucination checker failed: {str(e)}",
            "total_claimed": 0,
            "verified_count": 0,
            "unverified_count": 0,
